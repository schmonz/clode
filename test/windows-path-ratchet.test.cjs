...
// stripComments() and scanRegexBody() moved to test/strip-comments.cjs (2026-09-17,
// phase 4b task 1 fix round 1) once test/depscan.test.cjs needed the same tokenizer
// too: requiring one node:test file from another re-registers every test it
// declares, so a comment-masked source scan elsewhere in the suite could not just
// `require()` this file. The functions, and the FIX ROUND 1 / FIX ROUND 2 history
// inside stripComments()'s EOF-backoff branch, are unchanged -- see that file.
const { stripComments, scanRegexBody } = require('./strip-comments.cjs');
