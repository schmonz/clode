...
//      upstream's format drift" check. Providers are present only after `clode fetch claude`;
...
for (const v of VERSIONS) {
  test(`extract+inspect sha parity for ${v}`, (t) => {
    ...
    if (!bin) { t.skip(`no provider binary for ${v} (fetch with: clode fetch claude ${v})`); return; }
