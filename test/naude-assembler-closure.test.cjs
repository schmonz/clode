'use strict';
// The naude ASSEMBLER runs as loose scripts under a fetched node inside a blobulated
// clode-native (NO scripts/ dir of its own) — libexec/quaude-blobulate.js carries an
...
// Parsed from quaude-blobulate.js's builder-role loop so the test tracks the ACTUAL
...
function parseCarriedMembers(blobulateSrc) {
  const m = blobulateSrc.match(/for \(const f of \[([^\]]*)\]\)\s*\{\s*\n?\s*members\.push\(\{ name: `scripts\//);
  if (!m) {
    ...
    throw new Error('could not locate the naude-assembler member loop in quaude-blobulate.js — '
...
// both real and both measured: path.join(scriptsDir, 'x') in quaude-blobulate.js:318, and
...
// never run inside a blobulated builder. Scoped to the carried set plus its require-closure the
// same derivation reports ZERO findings on a clean tree and still finds both real sites.
function scriptsDirUses(blobulateSrc, scriptSources, members) {
  ...
  scan('quaude-blobulate.js', blobulateSrc);   // the worker is ALWAYS scanned
...
// PURE. Inputs: the quaude-blobulate.js source text, and a map of scripts/ filename -> source.
...
function scanAssembler({ blobulateSrc, scriptSources }) {
  const members = parseCarriedMembers(blobulateSrc);
  const findings = [];
  let examined = 0;
  ...
  // in quaude-blobulate.js (:318 reaches merge-step.mjs) and the same shape inside any carried
  // script (build-naude.mjs:415 reaches sea-sign.cjs). This is what catches a dropped LEAF.
  for (const [where, name] of scriptsDirUses(blobulateSrc, scriptSources, members)) {
    examined++;
    if (!members.includes(name)) {
      findings.push(`${where} reaches scripts/${name}, which is NOT a carried member `
        + `(a blobulated builder will die at that call with ENOENT)`);
    }
  }
...
test('regression: a dropped LEAF member is reported (BLIND until 2026-09-04)', () => {
  ...
  assert.deepStrictEqual(r.findings, [
    'quaude-blobulate.js reaches scripts/merge-step.mjs, which is NOT a carried member '
    + '(a blobulated builder will die at that call with ENOENT)',
  ]);
});
...
test('regression: a leaf reached from a CARRIED SCRIPT is reported too', () => {
  ...
  // quaude-blobulate.js would still be blind to this one.
  const r = scanAssembler({
    blobulateSrc: "for (const f of ['build-naude.mjs']) {\n    members.push({ name: `scripts/",
    scriptSources: {
...
test('a use site in an UNREACHABLE script is not a finding', () => {
  ...
  // is correct — they never run inside a blobulated builder. Scanning all of scripts/ produced
...
test('a member that IS carried and IS reached produces no finding', () => {
  const r = scanAssembler({
    blobulateSrc: "for (const f of ['build-naude.mjs', 'merge-step.mjs']) {\n    members.push({ name: `scripts/"
      + "\nconst p = path.join(scriptsDir, 'merge-step.mjs');",
    scriptSources: { 'build-naude.mjs': '', 'merge-step.mjs': '' },
  });
  assert.deepStrictEqual(r.findings, []);
  // Exact, not a loose lower bound: 1 use site (quaude-blobulate.js -> merge-step.mjs) + 2
...
test('regression: a carried member that does not exist is reported', () => {
  const r = scanAssembler({
    blobulateSrc: "for (const f of ['build-naude.mjs', 'ghost.cjs']) {\n    members.push({ name: `scripts/",
    scriptSources: { 'build-naude.mjs': '' },
  });
