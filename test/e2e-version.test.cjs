...
test('clode --version reports the shipped VERSION', (t) => {
  const sbx = sandbox(t);
  const r = spawnSync(NODE, [BIN, '--version'], { encoding: 'utf8', env: sbx.env });
  assert.strictEqual(r.status, 0);
  const version = fs.readFileSync(path.join(REPO, 'VERSION'), 'utf8').trim();
  assert.strictEqual((r.stdout || '').trim(), `clode ${version}`);
});

test('clode --clode-version (the old prefixed spelling) no longer dispatches', (t) => {
  const sbx = sandbox(t);
...
test('VERSION is a valid semver and LICENSE is BSD-2-Clause', () => {
  ...
  // value is covered by the --version + package.json-matches tests above.
  const version = fs.readFileSync(path.join(REPO, 'VERSION'), 'utf8').trim();
