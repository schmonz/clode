...
// shim in the loop, so with a real HOME and ANTHROPIC_API_KEY already set,
// naude-model calls the REAL `security find-generic-password -a <user> -w -s
// "Claude Code"/"Claude Code-credentials"` against the operator's real login
// Keychain — the "naming the operator's own account" dialog half of the original
// bug report. `security` is shadowed centrally now: test/run.mjs prepends a stub
// dir to `process.env.PATH` before this file (or any other) is even loaded, so
// `...process.env` below already carries the shadow — nothing file-local to add.
// See test/run.mjs's own comment for why a PREPENDED stub dir (not an emptied
// PATH) is required to shadow `security` without also losing rg/bfs/ugrep/git/sh
// resolution, and test/central-security-stub.test.cjs for the positive-control
// proof that a spawned child actually reaches the stub.
