...
const VERDICTS = [
  ...
  { name: 'CLODE_OFFLINE', verdict: 'env-only',
    because: 'declares that this run may not reach the network, and is READ (not just set) '
      ...
      + 'scripts/provision-bundle-inputs.sh refuses a cache miss rather than fetching; '
      + 'scripts/gen-unicode-data.cjs (CellSegmenter phase 3) likewise refuses a UCD cache '
      + 'miss with exit 3 instead of fetching, which test/unicode-data-fresh.test.cjs reports '
      + 'as a skip naming the file. It '
