...
test('unfloored (no floor) is bare and canonicalized; musl drops, glibc stays', () => {
  assert.strictEqual(flooredAssetName('windows-arm64', '0.1.2', ''), 'clode-0.1.2-windows-arm64.exe');
  assert.strictEqual(flooredAssetName('linux-x64-musl', '0.1.2', undefined), 'clode-0.1.2-linux-amd64');
  assert.strictEqual(flooredAssetName('linux-x64-glibc', '0.1.2', undefined), 'clode-0.1.2-linux-amd64-glibc');
});
