...
// file's readSiblingText(). Every OTHER tls test in this suite runs UNBLOBULATED
...
// libexec/quaude-blobulate.js's own collect(), which has no extension filter) and
// proves the blobulated path — not just the unblobulated one — resolves the real CA
// bundle, not an empty fallback.
test('VFS mount: tls.cjs resolves its blobulated sibling tls-cacert.pem (not the empty fallback)', (t) => {
  if (skipUnlessTjs(t)) return;
  ...
  assert.strictEqual(r.status, 0, r.stderr);
  const out = JSON.parse(r.stdout.trim());
  assert.ok(out.count >= 50, `expected >=50 blobulated root certs, got ${out.count}`);
