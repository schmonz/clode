...
// caution: size+mtime cannot see a same-size edit made inside the same second, and
// test/extract-cache-key.test.cjs now pins exactly that case. Measured cost: 0.41ms for
// all three sources (177KB) — nothing against a merge that runs for minutes.
//
// sigOf itself is deliberately untouched: cacheKey uses it on provider BINARIES (~50MB),
// where a hash per build would be a real cost and the mtime heuristic is fine — a provider
// is downloaded once and not regenerated byte-identically on every run.
